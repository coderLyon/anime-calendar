...
| `node scripts/verify/iqiyi-today.mjs` | 爱奇艺当天有数据（频道接口优先，浏览器逐日点击兜底） | 秒级（接口通道）/ 约 6 分钟（兜底） |
...
1. 爱奇艺优先走频道接口（`prelw/portal/lw/v5/channel/cartoon`「追番表」jmd_Mon~Sun），禁止把激活日 tab 的 force 点击作为主要手段（页面存在虚拟化/AB 变体，部分星期按钮可能不在 DOM）；
