...
- 发布前回归清单（`AGENTS.md` 亦收录）：`pnpm test`（Vitest 54 项）→ `pnpm build` → `work/app-qa.mjs`（14 项）→ `work/verify-m5.mjs`（22 项）→ `work/verify-m5b.mjs`（25 项）→ `work/m3-qa.mjs`（16 屏，需联网验证海报）→ `scripts/verify/`（youku-today / iqiyi-today / tencent-duration / data-fields）→ 线上抽查（部署 state=success + 首页/追番页无控制台错误）。
