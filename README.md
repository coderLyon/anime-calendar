...
Actions（cron 0 11,23 * * * UTC + workflow_dispatch + push）
  ...
  → node scripts/sync.mjs（抓取四平台 → 时长富集（B站季分集接口 / 腾讯卡片 / 优酷 show_page+播放页 / 爱奇艺 avlistinfo 分集接口）→ 短条目保留 + AI 短剧关键词/评论区启发式兜底 + 豆瓣甄别（优酷/爱奇艺缺失时长条目，限额防反爬）→ SVIP 抢先去重 → 最新集解析（腾讯按星期匹配卡片底部更新时间、结局点映等标签）→ 下周预计排期（未完结条目 +7 天推导）→ 写 data/updates.json）
