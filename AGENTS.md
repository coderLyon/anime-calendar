...
1. 爱奇艺优先走频道接口 `prelw/portal/lw/v5/channel/cartoon`（qyMesh 包装内「追番表」模块，jmd_Mon~Sun 7 组，一次性取回整周数据）；接口缺失/校验失败才回退浏览器逐日点击。**禁止把激活日 tab 的 force 点击作为主要手段**（历史根因：重复点击激活 tab 触发重载清空卡片；页面存在虚拟化变体时部分星期按钮不在 DOM）；
