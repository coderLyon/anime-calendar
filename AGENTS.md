...
- 追番为**平台级独立标记**：`follows` 仍按规范化标题共享一个条目（`platforms` 数组），但**同标题跨平台不自动互标**——看板星标、追番日历（仅已追番）、今日追番更新、浏览器提醒均按「平台 + 标题」精确匹配（`isFollowedOn`/`followedOn`）；用户在另一平台点星标才会追加该平台。
...
1. 爱奇艺优先走频道接口 `mesh.if.iqiyi.com/portal/lw/v7/channel/cartoon`（JSON 内「追番表」模块，jmd_Mon~Sun 7 组，一次性取回整周数据）；接口缺失/校验失败才回退浏览器逐日点击。**禁止把激活日 tab 的 force 点击作为主要手段**（历史根因：重复点击激活 tab 触发重载清空卡片；页面存在虚拟化变体时部分星期按钮不在 DOM）；
2. 当天 tab 默认激活时**不得重复点击**（先读后点，空则滚动重试，最后才「切走再切回」）；
3. 日期映射优先平台内联 date/week 字段；缺失时以「今/今天」tab 为锚推算，**禁止假定 tabs[0]=周一**；
4. 腾讯集数解析必须匹配带「集/话」后缀的编号（防「第2季」误当集数）；时长优先正片 vid 精确匹配；
5. `sync.mjs` 内置四平台「今天无真实条目」自检（写入 warnings，CI 输出 `::warning::`）；发布前运行 `scripts/verify/` 回归脚本（优酷秒级、爱奇艺接口通道秒级/浏览器兜底约 6 分钟、腾讯约 6 分钟，详见 `scripts/verify/README.md`）。
...
1. `pnpm test` —— Vitest 108 项（日期/筛选/短剧过滤/断更/历史归档/同步合并/规则与总集数解析/追番平台级标记/爱奇艺更新时间解析）；
...
- M5 迭代（`outputs/迭代计划书-v1.md` 已归档为完成基线：数据管道/PWA/基础 SEO/搜索筛选/多周导航/断更检测/本地追番与提醒引擎/Supabase 安全基础；不得把代码基础设施等同于端到端验收完成）
- V2 待审批实施（`outputs/迭代计划书-v2.md`）：云同步真实后端与跨设备闭环、通知授权入口与跨设备去重、完整 URL 深链/分享、数据边界加固及一次性设备配对；设计变更须先过 V2-G0，实施与发布继续执行完整回归清单
